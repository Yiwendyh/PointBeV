...
class AlignRes(nn.Module):
    ...
    def __init__(
        self,
        mode="upsample",
        scale_factors: Iterable[int] = [1, 2, 4],
        in_channels: Iterable[int] = [32, 64, 128],
    ):
        super().__init__()
...
class FPNNeck(nn.Module):
    ...
    def __init__(
        self,
        align_res_layer=None,
        prepare_c_layer=None,
...
if __name__ == "__main__":
    import torch
    out_1 = torch.randn((4, 32, 28, 60)).cuda()
    out_2 = torch.randn((4, 64, 14, 30)).cuda()
    out_3 = torch.randn((4, 128, 7, 15)).cuda()

    from collections import OrderedDict
    out = OrderedDict({'out_1':out_1, 'out_2':out_2, "out_3":out_3})
